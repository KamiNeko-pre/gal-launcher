...
- **一键转区与超分**：按游戏启用 Locale Emulator 或 Magpie，支持选择已有程序或从官方来源下载部署；超分提供四档方案及显卡建议，兼容性取决于游戏和设备。
- **批量导入**：选择一个总目录，收录第一层游戏文件夹；优先识别唯一包含 `chs` 的启动 EXE，无法确定时再确认。资料补全分为自动应用、待确认和未命中三档。
- **分类与排序**：自建分类、批量归类，在主页和各主题书架直接切换分类范围；支持题材筛选、标题和游玩时长等排序。
- **沉浸全屏与手柄**：用手柄切换游戏、浏览资料、进入书架、切换主题和退出全屏。
- **保留收藏**：删除游戏本体后仍保留资料和游玩记录，仅在点击启动时提示重新关联。
- **搜索与加载改进**：新增 GalgameWiki 中文资料源及来源标识，改进代理处理、翻译重试、横版图候选缓存和后台搜索反馈，减少启动时的整库图片预读。

完整变化见 [0.4.0 更新记录](CHANGELOG.md#040--2026-09-13)。
...
0.4.0 的 ZIP 解压根目录即有启动文件，请保留同目录下的资源。也可下载单文件 EXE 直接运行，无需手动解压。
